...
Judge::Judge()
{
    ...
    saveFile.open("/home/luoyuanfu/my_saved_file.txt",ios::app);
...
{
    //added by Yuanfu, to record results
    int elapsed = (current-start).toSec();
    int secs = elapsed%60;
    int mins = elapsed/60;
    if(mtype==PROPERLY_DETECTED) {
        num_detected_mines++;
        saveFile<<"num_mines: "<<num_detected_mines<<" time: "<<mins<<" "<<secs<<endl;
    }
...
void Judge::updateScoreboard()
{
    ...
    if(old_min != mins)
    {
     ...
     //   if(secs == 0) 
            //std::cout
            saveFile<<"min: "<<mins<<" "<<"second: "<<secs<<" "<<"coverage: "<<coverageRate<<" "
                <<"proper_detect: "<<properlyDetectedMines.markers.size()<<" "
                <<"wrong_detect: "<<wronglyDetectedMines.markers.size()<<" "
                <<"explosion: "<<knownExplodedMines.markers.size()+unknownExplodedMines.markers.size()<<" "
                <<"known_explosion: "<<knownExplodedMines.markers.size()<<" "
                <<"unknown_explosion: "<<unknownExplodedMines.markers.size()<<" "
                <<"visited_undetected: "<<visitedUndetectedMines.markers.size()<<" "
                <<"not_visited_undetected: "<<notVisitedUndetectedMines.markers.size()<<endl;
    }
